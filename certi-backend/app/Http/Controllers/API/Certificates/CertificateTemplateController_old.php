...
class CertificateTemplateController extends Controller
{
    ...
    protected CertificateFileService $fileService;
    protected CanvaValidationService $canvaValidationService;
    ...
    public function store(CertificateTemplateRequest $request): JsonResponse
    {
        try {
            ...
            if (Auth::check()) {
                ...
                // Asociar el usuario como propietario sin duplicar
                $template->owners()->syncWithoutDetaching([$user->id]);
            }
    ...
    /**
     * Actualizar posiciones de elementos en la plantilla
     *
     * @param Request $request
     * @param int $id
     * @return JsonResponse
    ...
    public function updatePositions(Request $request, $id): JsonResponse
    {
        try {
            ...
            $request->validate([
                ...
                'qr_position.x' => 'nullable|numeric',
                'qr_position.y' => 'nullable|numeric',
                'qr_position.width' => 'nullable|numeric',
                'qr_position.height' => 'nullable|numeric',
                'name_position' => 'nullable|array',
                'name_position.x' => 'nullable|numeric',
                'name_position.y' => 'nullable|numeric',
                'name_position.width' => 'nullable|numeric',
                'name_position.height' => 'nullable|numeric',
                'name_position.fontSize' => 'nullable|numeric',
                'name_position.fontFamily' => 'nullable|string',
                'name_position.fontWeight' => 'nullable|string',
                'name_position.color' => 'nullable|string',
                'background_image_size' => 'nullable|array',
                'background_image_size.width' => 'nullable|numeric',
                'background_image_size.height' => 'nullable|numeric',
            ]);

            $data = [];

            // Actualizar posición del QR si se proporciona
            if ($request->has('qr_position')) {
                $data['qr_position'] = $request->input('qr_position');
            }

            // Actualizar posición del nombre si se proporciona
            if ($request->has('name_position')) {
                $data['name_position'] = $request->input('name_position');
            }

            // Actualizar tamaño de imagen de fondo si se proporciona
            if ($request->has('background_image_size')) {
                $data['background_image_size'] = $request->input('background_image_size');
            }

            // Actualizar la plantilla
            $updatedTemplate = $this->templateService->update($template, $data);

            Log::info('Posiciones actualizadas para plantilla', [
                'template_id' => $template->id,
                'updated_data' => $data
            ]);
            ...
            return $this->successResponse([
                'template' => [
                    'id' => $updatedTemplate->id,
                    'qr_position' => $updatedTemplate->qr_position,
                    'name_position' => $updatedTemplate->name_position,
                    'background_image_size' => $updatedTemplate->background_image_size,
                    'updated_at' => $updatedTemplate->updated_at,
                ]
            ], 'Posiciones actualizadas exitosamente');
        } catch (\Exception $e) {
            Log::error('Error al actualizar posiciones: ' . $e->getMessage());
            return $this->errorResponse('Error al actualizar posiciones: ' . $e->getMessage(), 500);
        }
    }
}
